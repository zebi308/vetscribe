...
export async function loadPractices():Promise<Practice[]>{
...
practiceName:
profile.practice_name || undefined
...
export async function loadPlatformAnalytics(){
  ...
  const [
    ...
    db().from("practices").select("id",{count:"exact"}),
    db().from("profiles").select("id",{count:"exact"}),
    db().from("patients").select("id",{count:"exact"}),
    db().from("consultations").select("id",{count:"exact"})
  ]);
...
// ================================
// PHASE 13 FINAL WORKFLOWS
// ================================

export async function createAnalyticsEvent(payload:any){
