...
export default async function handler(req:any,res:any){if(!allowPost(req,res))return;try{const data=await openAIJson({system,input:req.body,schema:clinicalSchema,model:process.env.OPENAI_CLINICAL_MODEL||'gpt-4.1-mini'});res.status(200).json(data)}catch(e){res.status(401).json({error:e instanceof Error?e.message:'Clinical note generation failed'})}}
